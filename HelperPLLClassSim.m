...
if isempty(player)
   ...
   player = audioDeviceWriter('SampleRate',44100,'BufferSize',256);
   %Fs = 44100;
   startFreq = 80.06;
   fCenter = zeros(1,8);
   Kd = zeros(1,8);
...
if ~isempty(paramNew)
        for i =1:8
            ...
            Kd(i) = paramNew((i - 1) * 2 + 2);
        end
        
        if simControlFlags.resetObj % reset System objects
            %reset(reader);
            % Reset pitch shifter
            trackPitch(input,256,fCenter,Kd,Fs,true);
        end
end

%x = step(reader);
pitch =  trackPitch(in,256,fCenter,Kd,Fs);
step(player,in);
